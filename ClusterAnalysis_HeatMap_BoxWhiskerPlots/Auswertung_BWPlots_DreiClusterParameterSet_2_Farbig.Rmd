...
rownames(ilr_Genus_Data) <- substr(Genus_Data_norm[,1],1,3)
...
* Hier wurde am 29.04. verändert:
  + Kürzen der Probennamen auf die ersten drei Teile
  + Clusterzugehörigkeit in drei Farben
  + Geburtsweg kursiv!
```{r}
#KaiserSchnitt <- 
#  read.csv("Y:\\Auswertung_Datenanalyse\\DatenAnalyseMitR\\A_Daten\\20_OutPut_OutTables\\KaiserSchnitt.c#sv", sep = ";")
KaiserSchnitt <- 
  read.csv("Q:/NAS/IMiC/Auswertung_Datenanalyse/DatenAnalyseMitR/A_Daten/20_OutPut_OutTables/KaiserSchnitt.csv", sep = ";")
```
```{r}
# Geburtswegdaten 
KaiserSchnittFilt <- filter(KaiserSchnitt, ID %in%my.agnes$order.lab)

KaiserOrder <- KaiserSchnittFilt[my.agnes$order,]
face <- ifelse(KaiserOrder$Cesarean == 0, "plain","bold.italic")
...
dendro_plot <- ggdendrogram(data = dend, 
                              rotate = TRUE)
...
dendro_plot <-
dendro_plot +
  theme(axis.text.y = element_text(colour = c(rep("red",6),rep("blue",3),rep("grey30",4)),
                                   face = face))
...
# Preview the plot
  ...
  DataForHeatMap_long$ID_SHORT <- substr(DataForHeatMap_long$ID,1,3)
  # Re-order heatmap rows to match dendrogram
  dend_order <- order.dendrogram(dend)

  

  DataForHeatMap_long$ID <- factor(x = substr(DataForHeatMap_long$ID,1,3),
...
heatmap_plot <- ggplot(data = DataForHeatMap_long_extract, aes(x = measurement, y = ID)) +
  ...
  theme(axis.text.y = element_blank(),
        ...
        legend.position = "top") +
  geom_vline(xintercept = 0.5 + 0:14, colour = "black")+
  geom_hline(yintercept = 0.5 + 0:13, colour = "black")
heatmap_plot
```
...
```{r}
#pdf(paste(data_directory,"K_ClusterMitHeatmap_BEISPIEL/NurPatienten_14Spalten_mehrFarbe.pdf",sep = ""), 
...
```{r}
short_names <- function(x){substr(x,1,3)}
...
Kontrolle & Manuelle Zuordnung eines Bestandteils
```{r}
cluster_grouping
```
```{r}
table(cluster_grouping$ClusterNr)
```
...
for(aspect in avail_aspects){
  ...
  MetaData <- 
    data.frame(
      read_excel(paste(data_directory,
                       "/A_Daten/20210831_ClinicalData_OutputVersion_220411.xlsx",sep = "")))
  ...
  mdata <- MetaData[,c(1,3,index_of_interesting_data)]
  
  mdata$StudyID <- 
    substr(mdata$StudyID,6,8)
  
  # Infos zur ersten Visiten selektieren
  sel_md <- filter(mdata, Visit == 1)
  ...
  NA_index <- which(joined.meta.data[,4] == 33)
  if(length(NA_index) > 0){
    joined.meta.data[NA_index,4] <- NA 
  }
  
  x <- as.numeric(filter(joined.meta.data,ClusterNr == 1)[,4])
  y <- as.numeric(filter(joined.meta.data,ClusterNr == 2)[,4]) 
  ...
  x <- as.numeric(filter(joined.meta.data,ClusterNr == 1)[,4])
  y <- as.numeric(filter(joined.meta.data,ClusterNr == 3)[,4])                
  if(median(x, na.rm = TRUE) < median(y, na.rm = TRUE)){alt = "less"}else{alt = "greater"}
  pv_13 <-
    wilcox.test(x,y, alternative = alt)$p.val
  pv_13 <- round(pv_13,2)
  
  x <- as.numeric(filter(joined.meta.data,ClusterNr == 2)[,4])
  y <- as.numeric(filter(joined.meta.data,ClusterNr == 3)[,4])                
  if(median(x, na.rm = TRUE) < median(y, na.rm = TRUE)){alt = "less"}else{alt = "greater"}
  pv_23 <-
    wilcox.test(x,y, alternative = alt)$p.val
  ...
  boxplot(as.numeric(joined.meta.data[,4]) ~ round(joined.meta.data$ClusterNr,0),
